...
enum ImGuiFileBrowserFlags_ : std::uint32_t {
  ...
  ImGuiFileBrowserFlags_SkipItemsCausingError =
      1 << 10,  // when entering a new directory, any error will interrupt the
                ...
                // directory. with this flag, if an error is caused by a
                // specific item in the directory, that item will be skipped,
...
inline void
ImGui::FileBrowser::AssignToArrayStyleString(std::vector<char> &arr,
                                             std::string_view content) {
  if (content.empty()) {
    if (!arr.empty()) {
      arr[0] = '\0';
