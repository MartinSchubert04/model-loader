...
static void on_key_callback(GLFWwindow *window, int key, int scancode, int action, int mods) {
  auto pWindow = static_cast<window::Iwindow *>(glfwGetWindowUserPointer(window));
...
static void on_window_close_callback(GLFWwindow *window) {
  window::Iwindow *pWindow = static_cast<window::Iwindow *>(glfwGetWindowUserPointer(window));
  pWindow->onClose();
}
