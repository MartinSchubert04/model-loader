...
bool GLwindow::init(int width, int height, std::string title) {
  ...
  mPropertyPanel->setModel_load_callback([this](std::string filepath) { mScene->loadModel(filepath); });
...
void GLwindow::updateTitle() {
  ...
  glfwSetWindowTitle(mWindow, std::format("{} | FPS: {:.2f}", title, getFPS()).c_str());
}
